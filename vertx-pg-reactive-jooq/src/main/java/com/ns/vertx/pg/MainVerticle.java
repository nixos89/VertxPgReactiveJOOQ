...
//		routerREST.put("/books/:id").handler(this::updateBookHandler);
	...
	// TODO: fix handling of NON-EXISTING 'category_id' which has been sent in HTT GET request!!!
	...
	// TODO: fix handling of NON-EXISTING 'book_id' which has been sent in HTT GET request!!!
	private void getBookByIdHandler(RoutingContext rc) {
		long id = Long.valueOf(rc.request().getParam("id"));
		Future<JsonObject> future = BookJooqQueries.getBookByIdJooq(queryExecutor, id);
		future.setHandler(ok(rc));		
	}
	...
	private void createCategoryHandler(RoutingContext rc) {
		...
		CategoryJooqQueries
			.createCategoryJooq(queryExecutor, jo.getString("name"), jo.getBoolean("is_deleted"))
			.setHandler(ok(rc));
	}
	...
	private void createBookHandler(RoutingContext rc) {
		...
		LOGGER.info("(createBookHandler(..)) bookJO = " + bookJO.encodePrettily());
		BookJooqQueries
			.createBookJooqRetVal(queryExecutor, bookJO.getString("title"), bookJO.getDouble("price"), 
				bookJO.getInteger("amount"), bookJO.getBoolean("is_deleted"))
			.setHandler(ok(rc));
	}
	...
	private void updateCategoryHandler(RoutingContext rc) {
		...
		Category categoryPojo = new Category(catJO);
